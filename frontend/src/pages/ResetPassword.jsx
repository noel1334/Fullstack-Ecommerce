...
const ResetPassword = () => {
  ...
  const { BASE_URL } = useContext(UserContext); // Get the API base URL from context
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [email, setEmail] = useState(""); // Email for resending the link
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const [isLoading, setIsLoading] = useState(false); // Loading state
  const [isTokenValid, setIsTokenValid] = useState(false);
  const [isResending, setIsResending] = useState(false); // Resend link loading state
