...
export const createStripeSession = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        orderReference, // Only pass the reference
      },
    });

    // Save orderDetails in the database after session is successfully created
    const tempOrder = new Order({
      ...
      transactionId: session.id, // Save the Stripe session ID
    });
...
export const completeOrder = async (req, res) => {
  try {
    ...
    // Intentionally introduce an invalid value for testing
    ...
    try {
      ...
      // Attempt to delete the order if save fails
      try {
...
export const verifyMonnifyPaymentAndCreateOrder = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      try {
        // Save the Monnify transactionReference as transactionId
...
export const verifyFlutterwavePaymentAndCreateOrder = async (req, res) => {
  ...
  try {
    ...
    if (
      data.status === "success" &&
      data.data.tx_ref === tx_ref && // Compare transaction references
      data.data.amount === orderDetails.totalAmount && // Compare total amount
      data.data.currency === "NGN" // Ensure currency matches
    ) {
...
export const updateOrderStatusAndShippingStatus = async (req, res) => {
  try {
    ...
    const validOrderStatuses = ["Pending", "Processed", "Packing", "Shipped", "Progress", "Delivered", "Canceled"];
    if (orderStatus && !validOrderStatuses.includes(orderStatus)) {
      return res.status(400).json({ message: "Invalid order status provided." });
    }

    // Validate shipping status
    const validShippingStatuses = ["Processing", "Shipped", "In_Transit", "Out_For_Delivery", "Delivered", "Failed"];
    ...
    // Map shipping status to order status and calculate delivery date
...
// Update rejection details when the order is rejected
export const updateRejectedOrder = async (req, res) => {
  ...
  try {
    // Find and update the order with rejection details
    const order = await Order.findByIdAndUpdate(
      id,
      {
        acceptedOrder,
...
// Update Cancel details when the order is rejected
export const updateCancelOrder = async (req, res) => {
  ...
  try {
    // Find and update the order with cancellation details
    const order = await Order.findByIdAndUpdate(
      id,
      {
        cancelOrder,
